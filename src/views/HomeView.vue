...
const navigateToMovie = (movieId: number | null | undefined) => {
  if (!movieId) return
  router.push({ name: 'movies-details', params: { id: movieId } })
}

const goToUpcomingList = () => {
...
<template>
  ...
  <div class="px-5 py-20 lg:px-20">
    ...
    <div v-if="hasUpcoming" class="mt-10 grid gap-6 lg:grid-cols-[1.4fr_1fr]">
      <article
        class="relative overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-[#141414] to-[#050505] p-8">
        <img v-if="upcomingHero?.backdrop_path"
          :src="`https://image.tmdb.org/t/p/original${upcomingHero.backdrop_path}`" :alt="upcomingHero.title"
        ...
        <div class="relative z-10 flex flex-col gap-6">
          <div>
            ...
            <p class="mt-2 text-sm text-gray-200">Estreia em {{ formatReleaseDate(upcomingHero?.release_date) }}</p>
          </div>
          ...
          <div class="flex flex-wrap gap-4">
            <button
              ...
              @click="navigateToMovie(upcomingHero?.id)">
              Ver detalhes
            </button>
            <button
              class="rounded-full border border-white/30 px-6 py-2 text-sm font-semibold text-white hover:bg-white/5"
              @click="goToUpcomingList">
              Lembre-me
            </button>
          </div>
      ...
      <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-1">
        <article v-for="movie in upcomingGrid" :key="movie.id"
          class="flex gap-4 rounded-2xl border border-white/10 bg-[#0f0f0f] p-4">
          <img :src="movie.poster_path ? `https://image.tmdb.org/t/p/w300${movie.poster_path}` : '/placeholder.png'"
            :alt="movie.title" class="h-32 w-24 rounded-xl object-cover" />
    ...
    <div v-else class="mt-10 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      <article v-for="index in skeletonCards" :key="index"
      ...
      <div class="col-span-full rounded-3xl border border-white/10 bg-[#0c0c0c] p-6 text-center text-gray-400">
        Os próximos lançamentos estão sendo carregados. Volte em instantes ou explore a programação atual.
      </div>
    </div>
  </div>
</template>
